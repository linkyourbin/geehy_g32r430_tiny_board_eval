...
fn main() -> ! {
    ...
    rprintln!("SSD1306 I2C example started");
    rprintln!("I2C1 pins: PD5 = SCL, PD9 = SDA, external pull-ups required");
    rprintln!("SSD1306 7-bit address: 0x3C");
    ...
    loop {
        ...
        if let Err(error) = display.flush() {
            rprintln!("ssd1306 refresh failed: {:?}", error);
            board::delay_ms(100);
        }
